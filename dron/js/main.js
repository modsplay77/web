...
function inicio(){
	...
	for (i=0; i<j; i++){
		...
		`<div class="producto" id="h${i}" ><div class="nombre" onclick="ampliar(this)">${nombres[i]}</div><div class="foto" onclick="ampliar(this)"><img src="img/${miniaturas[i]}"></div><div class="descripcion">${descripciones[i]}</div></div>`;
	}
}
...
function repetir(){
	...
	document.getElementsByClassName("nombreampliar")[0].innerHTML=nombres[indice];
	document.getElementsByClassName("fotoamp")[0].innerHTML= `<div class="fotoamp"><img src="img/${imagenes[indice]}"</div>`;
	document.getElementsByClassName("descripcionamp")[0].innerHTML=descripciones[indice];
